...
def do_deploy(archive_path):
    ...
    if run('mkdir -p /data/web_static/releases/{}'.format(folder)).failed is True:
        return False
    if run('tar -xzf /tmp/{} -C /data/web_static/releases/{}/'.format(filename, folder), warn_only=True).failed is True:
        return False
    if run('rm /tmp/{}'.format(filename)).failed is True:
        return False
    if run('mv /data/web_static/releases/{}/web_static/* /data/web_static/releases/{}/'.format(folder, folder), warn_only=True).failed  is True:
        return False
    if run('rm -rf /data/web_static/current').failed is True:
        return False
    if run('ln -s /data/web_static/releases/{}/ /data/web_static/current'.format(folder)).failed is True:
        return False
    else:
